...
def allocate_fabric_rolls(docname):
    """
    Allocate fabric rolls using ONLY `fabric_requirement_against_marker` (length units).
    Resolves PRs via SO → MRP → PO → GRN → PR.
    Deducts previous allocations from other dockets.
    Uses roll_length or falls back to qty.
    """
    # Helper: Resolve PR for a Sales Order
    def get_pr_for_sales_order(so: str) -> str | None:
        if not so:
            return None
        ...
        po = frappe.db.get_value(
            "Purchase Order Item",
            {
                "custom_reference_parent_id": mrp_name,
    ...
    # Helper: Build PR state (roll lengths, usage, metadata)
    def ensure_pr_state(pr: str, docname: str, cache: dict):
        if pr in cache:
            return cache[pr]

        frappe.log_error(f"🔍 ensure_pr_state: Checking PR {pr}", "Cut Docket - PR State")
        ...
        frappe.log_error(f"📄 Fetched {len(items)} items from PR {pr}: {items}", "Cut Docket - PR Items")

        roll_len, meta, roll_numbers = {}, {}, []
        for it in items:
            rn = (it.roll_no or "").strip()
            if not rn:
                frappe.log_error(f"⚠️ Skipping item: empty roll_no in PR {pr}", "Cut Docket - Invalid Roll No")
                continue

            length = flt(it.roll_length)
            if length <= 0:
                length = flt(it.qty)
                frappe.log_error(f"🔁 Using qty={it.qty} as fallback for roll {rn} in PR {pr}", "Cut Docket - Fallback to Qty")
        ...
        if not roll_len:
            frappe.log_error(
                f"❌ No valid fabric rolls found in PR {pr}. "
                f"Check: item_group='Fabrics', custom_roll_no set, roll_length/qty > 0",
                "Cut Docket - No Rolls Found"
            )
            return None

        # Sum previous allocations from other Cut Dockets
        prev_rows = frappe.get_all(
            "Cut Docket Roll Allocation",
            filters={
                "docstatus": ["<", 2],
        ...
        state = {
            "roll_len": roll_len,
            "used": used,
            "order": sorted(set(roll_numbers)),
            "meta": meta
        }
        cache[pr] = state
        frappe.log_error(f"✅ PR {pr} state built: {state}", "Cut Docket - PR State Built")
        return state
    ...
    try:
        ...
        # 1. Validate requirement
        requirement = flt(doc.fabric_requirement_against_marker)
        if requirement <= 0:
            frappe.throw(_("Fabric Requirement Against Marker is zero or empty. Please enter a value before allocating."))

        frappe.log_error(f"🚀 Starting allocation for {docname}, Requirement: {requirement}", "Cut Docket - Allocation Start")

        # 2. Get unique Sales Orders
        sos = []
        seen_sos = set()
        for row in doc.table_size_ratio_qty or []:
            so = (row.sales_order or "").strip()
            if so and so not in seen_sos:
                sos.append(so)
                seen_sos.add(so)
        ...
        frappe.log_error(f"📋 Found Sales Orders: {sos}", "Cut Docket - Sales Orders")

        # 3. Resolve PRs via SO → MRP → PO → GRN → PR
        prs = []
        seen_prs = set()
        for so in sos:
            pr = get_pr_for_sales_order(so)
            if pr and pr not in seen_prs:
                prs.append((pr, so))
                seen_prs.add(pr)
                frappe.log_error(f"🔗 Resolved PR {pr} for SO {so}", "Cut Docket - PR Resolution")

        if not prs:
            frappe.throw(_("Could not resolve any Purchase Receipts from the Sales Orders."))

        # 4. Allocate across PRs
        pr_cache = {}
        remaining = requirement
        ...
        for pr, so in prs:
            ...
            frappe.log_error(f"📦 Allocating from PR {pr}: available rolls = {list(roll_len.keys())}", "Cut Docket - Allocation Loop")
            ...
            for rn in order:
                ...
                total_len = flt(roll_len.get(rn, 0.0))
                already_used = flt(used.get(rn, 0.0))
                available = max(0.0, total_len - already_used)
                if available <= 0:
                    continue

                alloc_len = min(available, remaining)
                used[rn] = already_used + alloc_len
                balance_after = total_len - used[rn]
                ...
                remaining -= alloc_len
                frappe.log_error(f"✂️ Allocated {alloc_len}m from roll {rn} (PR: {pr}), Remaining: {remaining}", "Cut Docket - Allocation Step")
        ...
        if remaining > 0:
            doc.append("table_roll_details", {
                ...
                "status": "",
                "remarks": f"Shortage of {remaining} length units",
                "purchase_receipt": prs[0][0]
            })
            frappe.msgprint(_("⚠️ Not enough roll length. Shortage: {0}").format(remaining), alert=True)
            has_error = True

        # 6. Save
        doc.save(ignore_permissions=True)

        msg = "✅ Fabric rolls allocated successfully." if not has_error else "⚠️ Allocation completed with shortage."
        frappe.msgprint(_(msg), alert=True)

    except Exception as e:
        frappe.log_error(f"Allocation failed for {docname}: {frappe.get_traceback()}", "Cut Docket - Roll Allocation Error")
        frappe.throw(_("Failed to allocate fabric rolls: {0}").format(str(e)))